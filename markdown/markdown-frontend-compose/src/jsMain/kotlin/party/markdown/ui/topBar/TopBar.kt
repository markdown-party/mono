...
fun TopBar(
  ...
  Div(
      attrs = {
        classes("p-4", "space-x-4", "bg-gray-100", "text-black", "flex", "flex-row", "items-center")
      }) {
    Img(
        src = "/img/logo.svg",
        attrs = {
          classes("h-12", "cursor-pointer", "hover:bg-gray-200", "rounded", "px-2", "py-1")
          onClick { onDebugEnabled(!debugEnabled) }
        })
    A(
      href = "https://github.com/markdown-party/mono",
      attrs = {
        classes("hover:bg-gray-200", "rounded", "p-3")
      }
    ) {
