...
class MemoryEcho<T>(
  ...
  override fun outgoing() =
      channelExchange<I<T>, O<T>> { incoming ->
        ...
        while (state != OutgoingState.Completed &&
            !(incoming.isClosedForReceive && isClosedForSend)) {
          state =
              when (val s = state) {
                is OutgoingState.Advertising ->
                    select {
                ...
                is OutgoingState.Cancelling -> select { onSend(O.Done) { OutgoingState.Completed } }

                // 1. We receive a Done message, and move to Completed.
                is OutgoingState.Completing ->
                    select {
                      incoming.onReceiveOrClosed { v ->
                        when (v.valueOrNull) {
  ...
  override fun incoming() =
      channelExchange<O<T>, I<T>> { incoming ->
        ...
        while (state != IncomingState.Completed &&
            !(incoming.isClosedForReceive && isClosedForSend)) {
          state =
              when (val s = state) {
                ...
                // 4. We receive an unsupported message, which we just ignore. // TODO : Fail fast
                // instead ?
                is IncomingState.New ->
                    select {
                ...
                is IncomingState.Sending ->
                    select {
                      ...
                      val event =
                          s.pendingEvents.firstOrNull { (id, _) ->
                            s.receivedCredits[id.site] ?: 0L > 0L
                          }
                      if (event != null) {
                        onSend(
                            I.Event(
                      ...
                      insertion.onReceive { _ -> mutex.withLock { s.update(log) } }
                    }

                // 1. We can send a Done message, and move to Completed.
                is IncomingState.Cancelling -> select { onSend(I.Done) { IncomingState.Completed } }

                // 1. We receive a Done message, and move to Completed.
                is IncomingState.Completing ->
                    select {
                      incoming.onReceiveOrClosed { v ->
                        when (v.valueOrNull) {
