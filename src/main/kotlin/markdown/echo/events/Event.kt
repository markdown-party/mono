    ...
    while (state != Completed && !(isClosedForSend && incoming.isClosedForReceive)) {
      state =
          when (val s = state) {
            ...
            is New ->
                select {
                  ...
                  incoming.onReceiveOrClosed { v ->
                    if (v.isClosed || v.valueOrNull == O.Done) Cancelling else New
                  }
                }
                    ...
                    when (val msg = v.valueOrNull) {
                      is O.Request ->
                          if (msg.site == site && msg.count > 0) {
                            ...
                            if (remaining.isNotEmpty()) Sending(remaining, msg.count) else Yielding
                          } else WaitingReady
                ...
                select {
                  ...
                  if (s.allowed >= 1) {
                    onSend(I.Event(site = site, seqno = seqno, body = body)) {
                      ...
                      if (remaining.isNotEmpty()) Sending(remaining, s.allowed - 1) else Yielding
                    }
                  }
                  incoming.onReceiveOrClosed { v ->
                    when (val msg = v.valueOrNull) {
            ...
            is Cancelling -> select { onSend(I.Done) { Completed } }

            // 1. We can receive a Done message, and move to Completed.
            is Completing ->
                select {
                  incoming.onReceiveOrClosed { v ->
                    if (v.isClosed || v.valueOrNull == O.Done) Completed else Completing
                  }
                }
