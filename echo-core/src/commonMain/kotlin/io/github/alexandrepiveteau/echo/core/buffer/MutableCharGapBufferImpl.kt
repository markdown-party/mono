...
/**
 * An implementation of [MutableCharGapBuffer]. The class implements [Gap] to avoid additional
...
internal class MutableCharGapBufferImpl : MutableCharGapBuffer, Gap {
  ...
  /**
   * Grows the [MutableCharGapBuffer] backing [buffer] to the given [size]. If the provided [size]
  ...
  override val backing: CharArray
    get() = buffer
  ...
  override fun set(offset: Int, value: Char) {
    // Preconditions.
    requireIn(offset, 0, this.size)
  ...
  override fun push(value: Char, offset: Int) {
    // Preconditions.
  ...
  override fun push(array: CharArray, offset: Int, startIndex: Int, endIndex: Int) {
    // Range preconditions
  ...
  override fun copyInto(
    ...
    // calculated, with the following pattern when copying chars :
  ...
  /**
   * Outputs a [String] representation of the [MutableCharGapBuffer]. Chars which are part of the
   * gap will be prefixed by a '*' symbol.
   */
  override fun toString(): String {
    val content = gap.bufferToString(buffer.toTypedArray())
    return "MutableCharGapBuffer(start=$startIndex, end=$endIndex, data=[${content}])"
  }
}
